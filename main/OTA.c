...
static bool connect_to_http_server()
{
	...
	if (socket_id == -1) {
		ESP_LOGE(TAG_OTA, "Create socket failed!");
		return false;
	}
	...
	if (http_connect_flag == -1) {
		ESP_LOGE(TAG_OTA, "Connect to server failed! errno=%d", errno);
		close(socket_id);
		return false;
	}
	else {
...
bool _esp_ota_firm_parse_http(esp_ota_firm_t *ota_firm, const char *text, size_t total_len, size_t *parse_len)
{
	...
	while (text[i] != 0 && i < total_len) {
		...
		if (i_read_len > total_len - i) {
			ESP_LOGE(TAG_OTA, "recv malformed http header");
			task_fatal_error();
		}
		...
		if(i_read_len == 2) {
			if (ota_firm->content_len == 0) {
				ESP_LOGE(TAG_OTA, "did not parse Content-Length item");
				task_fatal_error();
			}
...
static size_t esp_ota_firm_do_parse_msg(esp_ota_firm_t *ota_firm, const char *in_buf, size_t in_len)
{
	...
	case ESP_OTA_INIT:
		if (_esp_ota_firm_parse_http(ota_firm, in_buf, in_len, &tmp)) {
			...
			ESP_LOGD(TAG_OTA, "Http parse %d bytes", tmp);
			parsed_bytes = tmp;
		}
		break;
	case ESP_OTA_PREPARE:
		...
		if (ota_firm->read_bytes >= ota_firm->ota_offset) {
			...
			ESP_LOGD(TAG_OTA, "Receive %d bytes and start to update", ota_firm->read_bytes);
			ESP_LOGD(TAG_OTA, "Write %d total %d", ota_firm->bytes, ota_firm->write_bytes);
		}
	...
	default:
		parsed_bytes = 0;
		ESP_LOGD(TAG_OTA, "State is %d", ota_firm->state);
		break;
	}
...
static void esp_ota_firm_init(esp_ota_firm_t *ota_firm, const esp_partition_t *update_partition)
{
	...
	ESP_LOGI(TAG_OTA, "Totoal OTA number %d update to %d part", ota_firm->ota_num, ota_firm->update_ota_num);
...
void ota_example_task(void *pvParameter)
{
	...
	ESP_LOGI(TAG_OTA, "Starting OTA example... @ %p flash %s", ota_example_task, CONFIG_ESPTOOLPY_FLASHSIZE);
